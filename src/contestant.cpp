#include "contestant.h"
#include "item.h"
#include "Arena.h"
#include "Codemon.h"
#include "mechanic.h"
#include <iostream>
#include <string>
using namespace std;

void Contestant::resizeQueue(int newCapacity) {
	Item* newQueue = new Item[newCapacity];
	for (int i = 0; i < qSize; i++) {
		newQueue[i] = itemQueue[(qFront + i) % qCapacity];
	}
	delete[] itemQueue;
	itemQueue = newQueue;
	qCapacity = newCapacity;
	qFront = 0;
}

void Contestant::placeRandomCodemon(Arena& A) {
	do {
		row = rand() % 12;
		col = rand() % 12;
	} while (A.isOccupied(row, col));
	return;
}

void Contestant::setActiveCodemon(const int n) {
	activeCodemon = n;
}

Contestant::Contestant() {
	playerName = " ";
	isHuman = true;
	row = -1;
	col = -1;
}

Codemon* Contestant::getTeam() const { 
    return team; 
}

Contestant::Contestant(string name, bool human, int codemonNum) {
	playerName = name;
	isHuman = human;
	setTeamSize(codemonNum);
	activeCodemon = 0;
	row = -1;
	col = -1;
	team = new Codemon[teamsize];
	void placeRandomCodemon(Arena & A);
}

Contestant::~Contestant() {
	delete[] team;
}

Contestant::Contestant(const Contestant& other) {
	cout << "Copying Contestant: " << other.playerName << endl;
	cout << "Enter name for cloned Contestant: ";
	getline(cin, playerName);

	teamsize = other.teamsize;
	activeCodemon = other.activeCodemon;
	row = other.row;
	col = other.col;
	isHuman = other.isHuman;

	team = new Codemon[teamsize];
	for (int i = 0; i < teamsize; ++i) {
		team[i] = other.team[i];
	}
}

Contestant& Contestant::operator = (const Contestant& rhs) {
	if (this == &rhs) {
		return *this;
	}

	delete[] team;

	this->playerName = rhs.playerName;
	this->teamsize = rhs.teamsize;
	this->activeCodemon = rhs.activeCodemon;
	this->row = rhs.row;
	this->col = rhs.col;
	this->isHuman = rhs.isHuman;

	team = new Codemon[teamsize];
	for (int i = 0; i < teamsize; ++i) {
		team[i] = rhs.team[i];
	}
	return *this;
}

void Contestant::initSharedQueue(int firstCap) {
	delete[] itemQueue;
	itemQueue = new Item[firstCap];
	qCapacity = firstCap;
	qSize = 0;
	qFront = 0;
}

string Contestant::getName() const{
    return playerName;
}

int Contestant::getRow() const {
    return row;
}

int Contestant::getCol() const{ 
    return col;
}

bool Contestant::isHum() const{
    return isHuman;
}

void Contestant::setTeamSize(int n) {
	delete[] team;
	teamsize = n;
	activeCodemon = 0;
	team = new Codemon[teamsize];
}

bool Contestant::addCodemon(const Codemon& c, int idx) {
	if (idx < 0 || idx >= teamsize) return false;
	team[idx] = c;
	return true;
}

Codemon& Contestant::getActive() {
	return team[activeCodemon];
}

void Contestant::spawn(Arena& arena, int r, int c, char symbol) {
	row = r;
	col = c;
	this->symbol = symbol;
	arena.setTerrainTile(row, col, symbol);
}

void Contestant::takeTurn(Arena& arena, char symbol) {
	if (isHuman) {
		arena.printFullVisibleMap(getRow(), getCol());
		cout << "[" << playerName << "] move (W A S D Q E Z C): ";
		char mv;
		cin >> mv;
		movement(mv, symbol, arena);
		row = arena.getCurRow();
		col = arena.getCurCol();
	}
	else {
		const char dir[8] = { 'W','A','S','D','Q','E','Z','C' };
		char mv = dir[rand() % 8];
		movement(mv, symbol, arena);
	}
	row = arena.getCurRow();
	col = arena.getCurCol();
}

void Contestant::enqueueItem(const Item& item) {
	if (qSize == qCapacity) {
		resizeQueue(qCapacity * 2);
	}
	int pos = (qFront + qSize) % qCapacity;
	itemQueue[pos] = item;
	qSize++;
}

bool Contestant::useNextItem() {
	if (qSize == 0){
	    return false;
	}
	Item& item = itemQueue[qFront];
	qFront = (qFront + 1) % qCapacity;
	qSize--;
	
	if (qCapacity > 4 && qSize < qCapacity / 4){
	   resizeQueue(qCapacity / 2); 
	}
	
	cout << "Using item: ";
	item.print();
	
	Codemon &active = getActive();
    string type = item.getEffectType();
    int amt     = item.getEffectAmount();
    
    if (type == "Heal") {
        active.heal(amt);
        cout << active.getName() << " healed for " << amt << " hp.\n";
    }
    else if (type == "Boost") {
        // we’ll temporarily shim this onto your Codemon: 
        // assume you added a method: void Codemon::increaseDamage(int)
        active.increaseDamage(amt);
        cout << active.getName() << "'s next attack boosted by +" << amt << ".\n";
    }
    else if (type == "Shield") {
        // assume you added a method: void Codemon::setShield(int)
        active.setShield(amt);
        cout << active.getName() << " gains a shield of " << amt << " hp.\n";
    }
	
	return true;
}

void Contestant::printVisibleMap(const Arena& A) const{
    const int RAD  = 2;                       // 5×5
    const int SIZE = Arena::SIZE;

    struct Seen { char sym; int r,c; };       // what we’ll list later
    Seen legend[25];
    int  seenCnt = 0;

    for (int r = row - RAD; r <= row + RAD; ++r) {
        for (int c = col - RAD; c <= col + RAD; ++c) {

            char out = '#';                                   // fog

            if (r>=0 && r<SIZE && c>=0 && c<SIZE) {
                if      (r == row && c == col){
                    out='@';
                } else if (r == A.getEneRow() && c == A.getEneCol()){
                    out='E';
                } else if (r == A.getLastKnownRow() && c == A.getlastKnownCol()){
                    out='?';
                } else if (A.hasItemAt(r,c)){
                    out='I';
                } else if (A.isOccupied(r,c)){
                    out='A';
                } else{
                    out=A.getTerrainTile(r,c);
                }
                
                if (out!='#' && out!='@' && seenCnt<25) {      // store for legend
                    legend[seenCnt++] = {out,r,c};
                }
            }
            cout << out << ' ';
        }
        cout << endl;
    }
    cout << "=========" << endl;

    //Codemon
    const Codemon& me = getActive();
    cout << "[@] " << playerName << "'s " << me.getName()
              << " is standing on " << A.getTerrainTile(row,col) << ".\n"
              << "Type: "  << me.getType()
              << "\nLevel: "<< me.getLevel()
              << "\nHP: "   << me.getCurrentHP() << '/' << me.getMaxHP()
              << "\n\n";
}



bool Contestant::isAlive() const {
	return !(team[activeCodemon].isFainted());
}

void Contestant::selectCodemons(Codemon pool[], int poolSize, bool random) {

	if (random) {
		for (int i = 0; i < teamsize; i++)
			team[i] = pool[rand() % poolSize];
	}
	else {
		for (int i = 0; i < teamsize; i++) {
			cout << "\nPick Codemon #" << i + 1 << ":\n";
			for (int j = 0; j < poolSize; ++j) {
				cout << j << ": " << pool[j].getName() << " (" << pool[j].getType() << ")\n";
			}
			int choice;
			cout << endl << "pick a number from the pool: ";
			cin >> choice;
			while (choice < 0 || choice >= poolSize) {
				cout << "Invalid. Try again: ";
				cin >> choice;
			}
			team[i] = pool[choice];
		}
	}
}

const Codemon& Contestant::getActive() const {   
    return team[activeCodemon];
}

// Static member initialization
Item* Contestant::itemQueue = nullptr;
int   Contestant::qCapacity = 0;
int   Contestant::qSize = 0;
int   Contestant::qFront = 0;